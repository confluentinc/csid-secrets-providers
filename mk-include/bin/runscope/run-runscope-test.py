...
def main():
    ...
    if trigger_resp.ok:
        ...
        if fail_count > 0:
            print("{} test runs passed. {} test runs failed.".format(
...
def _get_result(test_run):
    ...
    if not "RUNSCOPE_ACCESS_TOKEN" in os.environ:
        print(
            "Please set the environment variable RUNSCOPE_ACCESS_TOKEN. You can get an access token by going to https://www.runscope.com/applications"
        )
        exit(1)
    ...
    result_url = "{base_url}/buckets/{bucket_key}/tests/{test_id}/results/{test_run_id}".format(
        **opts)
