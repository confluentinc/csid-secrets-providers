...
__with_backoff() {
  ...
  do
    ...
    # We'll exponentially increase our timeout with some jitter
    timeout=$(awk -v min=1.5 -v max=2.3 -v timeout=$timeout 'BEGIN{srand(); print timeout*(min+rand()*int(1000*(max-min)+1)/1000)}')
  done

  if [[ $exitCode != 0 ]]
  then
    echo "Failed too many times, giving up... ($@)" 1>&2
  fi
...
__check_eks_status() {
  ...
  __with_backoff aws eks describe-cluster \
      --region "${aws_region}" \
      --name "${eks}" \
...
__configure_kubecfg_parallel_run() {
  ...
  "${list_clusters_func}" "${list_clusters_arg}" | \
    xargs -I{} -P"${CONFIGURE_KUBECFG_PARALLEL}" \
      "${_shell}" -c "
        $(declare -f __with_backoff);
        $(declare -f __check_eks_status);
...
__configure_kubecfg_aws_eks_single_cluster() {
  ...
  if ! __check_eks_status "${aws_region}" "${eks}"; then
    echo "WARNING: Skipping '${eks}' because status is not ACTIVE" 1>&2
    return 0
  fi

  [ -n "${VERBOSE:-}" ] && echo "Configuring eks kubectl ${eks_cluster}" 1>&2
  if ! KUBECONFIG="${target_kubeconfig}" \
    __with_backoff aws eks update-kubeconfig \
      --region "${aws_region}" \
      --alias "${eks}" \
...
__configure_kubecfg_aws_eks_list_clusters() {
  ...
  local list_clusters=$(__with_backoff aws eks list-clusters --region "${aws_region}")
  echo $list_clusters | jq --arg aws_region "${aws_region}" -r '$aws_region + ":" + .clusters[]'
}
...
__configure_kubecfg_aws_eks() {
  ...
  if ! __with_backoff __configure_kubecfg_aws_eks_is_supported "${aws_region}"; then
