...
@Description("This config provider is used to retrieve secrets that are mounted to the current container in Kubernetes.")
...
@ConfigProviderKey("k8sSecret")
public class K8sSecretConfigProvider extends AbstractConfigProvider<K8sSecretConfigProviderConfig> {
  ...
  protected Map<String, String> getSecret(SecretRequest secretRequest) throws Exception {
    ...
    log.info("Secret Directory: {}", secretDirectory);
    ...
    for (File key : secretDirectory.listFiles(File::isFile)) {
      log.debug("Reading {} to key {}", key.getAbsolutePath(), key.getName());
